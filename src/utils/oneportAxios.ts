...
function getBasicStructure(): any {
  return {
      path: REGISTRY_PATH,
      type: 'directory',
...
async function getComponentMetadata(componentName: string): Promise<any> {
  try {
    ...
    const files = filesMatch?.[1] 
      ? filesMatch[1]
          .match(/path:\s*["']([^"']+)["']/g)
          ?.map((pathMatch: string) => pathMatch.match(/path:\s*["']([^"']+)["']/)?.[1])
    ...
    return {
      ...
      files: files.map((path: string) => ({ path }))
    };
  } catch (error) {
...
async function getAvailableBlocks(category?: string): Promise<any> {
  ...
  try {
    ...
    const treeItems = response.data.filter((item: any) => item.type === 'tree');
